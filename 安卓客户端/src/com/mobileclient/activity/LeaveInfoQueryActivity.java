...
public class LeaveInfoQueryActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		leaveClassObj_ShowText = new String[leaveClassCount+1];
		leaveClassObj_ShowText[0] = "不限制";
		for(int i=1;i<=leaveClassCount;i++) { 
			leaveClassObj_ShowText[i] = leaveClassList.get(i-1).getLeaveClassName();
		} 
		...
		spinner_leaveClassObj.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,int arg2, long arg3) {
				if(arg2 != 0)
					queryConditionLeaveInfo.setLeaveClassObj(leaveClassList.get(arg2-1).getLeaveClassId()); 
				else
					queryConditionLeaveInfo.setLeaveClassObj(0);
			}
			@Override
		...
		spinner_studentObj.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,int arg2, long arg3) {
				if(arg2 != 0)
					queryConditionLeaveInfo.setStudentObj(studentList.get(arg2-1).getStudentNo()); 
				else
					queryConditionLeaveInfo.setStudentObj("");
			}
			@Override
			public void onNothingSelected(AdapterView<?> arg0) {}
		});
		...
		btnQuery.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				try {
					...
					queryConditionLeaveInfo.setAddTime(ET_addTime.getText().toString());
					queryConditionLeaveInfo.setLeaveState(ET_leaveState.getText().toString());
