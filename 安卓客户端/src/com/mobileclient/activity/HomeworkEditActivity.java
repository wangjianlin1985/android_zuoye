...
public class HomeworkEditActivity extends Activity {
			...
			public void onClick(View v) {
				try {
					...
					if(ET_title.getText().toString().equals("")) {
						Toast.makeText(HomeworkEditActivity.this, "作业标题输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_content.getText().toString().equals("")) {
						Toast.makeText(HomeworkEditActivity.this, "作业内容输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_hwRequire.getText().toString().equals("")) {
						Toast.makeText(HomeworkEditActivity.this, "作业要求输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传作业信息*/
					HomeworkEditActivity.this.setTitle("正在更新作业信息，稍等...");
					String result = homeworkService.UpdateHomework(homework);
	...
	private void initViewData() {
		...
		for (int i = 0; i < courseList.size(); i++) {
			if (homework.getCourseObj().equals(courseList.get(i).getCourseNo())) {
				this.spinner_courseObj.setSelection(i);
				break;
			}
		}
		this.ET_title.setText(homework.getTitle());
		this.ET_content.setText(homework.getContent());
		this.ET_hwRequire.setText(homework.getHwRequire());
