...
public class TeacherAction extends BaseAction {
    ...
    public String QueryTeacher() {
        ...
        if(name == null) name = "";
        if(comeDate == null) comeDate = "";
        if(postName == null) postName = "";
        if(telephone == null) telephone = "";
        List<Teacher> teacherList = teacherDAO.QueryTeacherInfo(teacherNo, name, comeDate, postName, telephone, currentPage);
        /*计算总的页数和总的记录数*/
        teacherDAO.CalculateTotalPageAndRecordNumber(teacherNo, name, comeDate, postName, telephone);
        /*获取到总的页码数目*/
    ...
    public String QueryTeacherOutputToExcel() { 
        ...
        if(postName == null) postName = "";
        if(telephone == null) telephone = "";
        List<Teacher> teacherList = teacherDAO.QueryTeacherInfo(teacherNo,name,comeDate,postName,telephone);
        ExportExcelUtil ex = new ExportExcelUtil();
        String title = "Teacher信息记录"; 
        String[] headers = { "教师编号","姓名","性别","年龄","入职日期","职称","联系电话"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<teacherList.size();i++) {
        	Teacher teacher = teacherList.get(i); 
        	dataset.add(new String[]{teacher.getTeacherNo(),teacher.getName(),teacher.getSex(),teacher.getAge() + "",new SimpleDateFormat("yyyy-MM-dd").format(teacher.getComeDate()),teacher.getPostName(),teacher.getTelephone()});
        }
        /*
