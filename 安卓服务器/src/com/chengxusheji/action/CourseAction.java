...
public class CourseAction extends BaseAction {
    ...
    public String QueryCourse() {
        ...
        List<Course> courseList = courseDAO.QueryCourseInfo(courseNo, courseName, teacherObj, coursePlace, currentPage);
        /*计算总的页数和总的记录数*/
        courseDAO.CalculateTotalPageAndRecordNumber(courseNo, courseName, teacherObj, coursePlace);
        /*获取到总的页码数目*/
        ...
        List<Teacher> teacherList = teacherDAO.QueryAllTeacherInfo();
        ctx.put("teacherList", teacherList);
        ctx.put("coursePlace", coursePlace);
        return "query_view";
    }
    ...
    public String QueryCourseOutputToExcel() { 
        ...
        if(courseName == null) courseName = "";
        if(coursePlace == null) coursePlace = "";
        List<Course> courseList = courseDAO.QueryCourseInfo(courseNo,courseName,teacherObj,coursePlace);
        ExportExcelUtil ex = new ExportExcelUtil();
        String title = "Course信息记录"; 
        String[] headers = { "课程编号","课程名称","上课老师","上课地点","上课时间","总学时"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<courseList.size();i++) {
        	Course course = courseList.get(i); 
        	dataset.add(new String[]{course.getCourseNo(),course.getCourseName(),course.getTeacherObj().getName(),
course.getCoursePlace(),course.getCourseTime(),course.getCourseHours() + ""});
        }
        /*
