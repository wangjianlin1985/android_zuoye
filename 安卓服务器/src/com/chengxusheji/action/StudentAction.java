...
public class StudentAction extends BaseAction {
    ...
    public String QueryStudent() {
        ...
        List<Student> studentList = studentDAO.QueryStudentInfo(studentNo, name, birthday, telephone, currentPage);
        /*计算总的页数和总的记录数*/
        studentDAO.CalculateTotalPageAndRecordNumber(studentNo, name, birthday, telephone);
        /*获取到总的页码数目*/
    ...
    public String QueryStudentOutputToExcel() { 
        ...
        if(birthday == null) birthday = "";
        if(telephone == null) telephone = "";
        List<Student> studentList = studentDAO.QueryStudentInfo(studentNo,name,birthday,telephone);
        ExportExcelUtil ex = new ExportExcelUtil();
        String title = "Student信息记录"; 
        String[] headers = { "学号","姓名","性别","学生照片","出生日期","家长电话"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<studentList.size();i++) {
        	Student student = studentList.get(i); 
        	dataset.add(new String[]{student.getStudentNo(),student.getName(),student.getSex(),student.getStudentPhoto(),new SimpleDateFormat("yyyy-MM-dd").format(student.getBirthday()),student.getTelephone()});
        }
        /*
